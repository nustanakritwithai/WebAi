...
const checks = [
  ...
  ["new artifact files use optional reads and revision zero", core.includes("readTaskFileIfPresent") && core.includes("writeFile.expectedRevision = 0") && workspace.includes("readTaskFilesOptional")],
  ["agent generation saves each plan step immediately without an Apply step", core.includes('task.status = "applying"') && core.includes("executeBrowserPlanSteps") && core.includes('source: "browser-agent-step"') && core.includes("readTaskFileIfPresent(workspace, task.id, file.name)") && !core.includes("applyAgentFiles") && !html.includes("applyAgentFilesBtn")],
  ["step completion requires complete readback metadata", core.includes("readbackEvidenceForRecord") && core.includes("contentPresent") && core.includes("contentBytes") && core.includes("version: metadataComplete ? revision") && core.includes("hash && /^[a-f0-9]{64}$/") && core.includes("record.hash !== revisions[0]?.hash") && core.includes("Number(file.version) === Number(file.revision)")],
  ...
  ["local JavaScript aliases resolve to the saved app.js file", core.includes("normalizeBrowserScriptReference") && core.includes('"$1app.js$2"') && core.includes("sanitizeBrowserPreviewFile(rawContent, file.name)")],
  ...
  ["follow-up keeps existing saved revisions until replacement artifacts are ready", core.includes("Preserve the last saved revisions") && core.includes("task.artifactManifest = Array.isArray(task.artifactManifest)") && !core.includes("task.artifactManifest = [];\n  task.artifactProgress")],
  ...
  ["step completion requires readback evidence and valid dependencies", core.includes("browserStepEvidenceIsValid") && core.includes("validateBrowserPlanDependencies") && core.includes('phase: "plan_invalid"')],
  ["preview failures remain retryable", core.includes('state.agentTask.status = "awaiting_preview"') && core.includes('phase: "preview_failed"') && core.includes("persistBrowserTaskState(workspace, state.agentTask)")],
