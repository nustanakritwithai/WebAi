...
function composeWorkspaceDocument(files, token) {
  ...
  const frameStyle = `<style id="webai-preview-frame-style">html,body{width:100%!important;min-width:100%!important;height:100%!important;min-height:100%!important}body{box-sizing:border-box;margin:0!important;overflow:auto}body>:first-child.webai-fluid-root,#root.webai-fluid-root,#app.webai-fluid-root,main.webai-fluid-root,.webai-fluid-root{box-sizing:border-box;min-height:100%!important}</style>`;
  const frameLayoutRuntime = `<script>(function(){function hasDeclaredSize(node,property){var inline=node.style||{};if(inline[property]&&inline[property]!=='auto')return true;for(var sheet of Array.from(document.styleSheets)){var rules;try{rules=sheet.cssRules||[];}catch(_){continue;}for(var rule of Array.from(rules)){if(rule.style&&rule.selectorText){try{if(node.matches(rule.selectorText)&&rule.style[property]&&rule.style[property]!=='auto')return true;}catch(_){}}}}return false;}function hasExplicitSize(node){return !node||node.hasAttribute('data-webai-fixed-size')||hasDeclaredSize(node,'height')||hasDeclaredSize(node,'minHeight');}function markFluid(node){if(!node||hasExplicitSize(node))return;node.classList.add('webai-fluid-root');}function applyFluidFrame(){var roots=new Set(Array.from(document.querySelectorAll('body>:first-child,#root,#app,main')));roots.forEach(markFluid);}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',applyFluidFrame,{once:true});else applyFluidFrame();})();</script>`;
